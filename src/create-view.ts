...
import { WORKSPACE_FOLDERS, workspaceManager, FileType } from './workspace-manager';
import { viewFileExtension } from './config';
...
export function createView(): void {
	...
	const id = ++viewId;
	const viewFileName = `${id}.${viewFileExtension}`;
	const viewFilePath = path.join(rootPath, WORKSPACE_FOLDERS[FileType.VIEW], viewFileName);
	...
	const viewModelInterfaceFilePath = path.join(rootPath,
		WORKSPACE_FOLDERS[FileType.VIEW_MODEL_ITF], viewModelInterfaceFileName);
	...
	const mockFilePath = path.join(rootPath,
		WORKSPACE_FOLDERS[FileType.VIEW_MODEL_MOCK], mockFileName);
